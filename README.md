...
# Run wallet management example
./examples/wallet_management

# Run token operations example
./examples/token_operations
...
- **Token operations** - Creating tokens, transfers, and balance verification
- **Metadata management** - Asset metadata and storage
- **Complete workflow** - End-to-end transaction examples
...
- Modern C++20 memory safety features
...
- 🐛 Issues: [GitHub Issues](https://github.com/WishKnish/KnishIO-Client-CPP/issues)
...
The C++ SDK maintains 100% compatibility with the JavaScript SDK through comprehensive test vectors. These manually-curated test vectors ensure consistent behavior across platforms.
...
find_package(KnishIO REQUIRED)

# Link to your target
target_link_libraries(your_target PRIVATE KnishIO::Client)
```
...
```bash
# Static linking
g++ -std=c++20 -o myapp myapp.cpp -lknishio-client -lsodium -lcurl -lpthread

# Dynamic linking
g++ -std=c++20 -o myapp myapp.cpp -lknishio-client-shared -lsodium -lcurl -lpthread
```
...
sudo apt-get install build-essential cmake libsodium-dev libcurl4-openssl-dev
...
See also the list of [contributors](https://github.com/WishKnish/KnishIO-Client-CPP/contributors) who participated in this project.
