#!/usr/bin/env bash
#
# Version consistency gate for the KnishIO C++ SDK.
#
...
collect "KnishIOClient.cpp SDK_VERSION" \
  "$(grep -oE 'SDK_VERSION *= *"[0-9]+\.[0-9]+\.[0-9]+"' src/KnishIOClient.cpp | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)"
# Third source, previously unchecked by the publish gate. This is the literal the cross-SDK
# gauntlet's snapshot coherence gate reads out of cpp-results.json.
collect "self-test-compatible.cpp version" \
  "$(grep -oE 'version *= *"[0-9]+\.[0-9]+\.[0-9]+"' self-test-compatible.cpp | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)"
# -----------------------------------------------------------------------------------------
